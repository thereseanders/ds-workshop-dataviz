...
We use the [`broom` package](https://cran.r-project.org/web/packages/broom/vignettes/broom.html) to convert the results of our model to a tidy data frame. 
...
**Exercise**: Make the plot prettier by flipping the axes.

```{r, warning = F, message = F, fig.height = 5, fig.width = 5, echo=F}
df_mod1 %>% 
  dplyr::filter(term != "(Intercept)") %>% 
  ggplot(aes(x = term, y = estimate)) +
