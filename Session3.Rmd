...
```{r,  warning = F, message = F, fig.height = 2, fig.width = 5, include = FALSE}
departures <- data %>%
...
```{r, warning = F, message = F, fig.height = 2, fig.width = 6, include = FALSE}
# There are a number of ways to improve the plot. We will go through them step by step. There is a weird observation in hour 24 that should not be there. The hour has to be either 23 or 0. Lets re-code this observation to an hour value of 0 using the `replace()` function from `dplyr`.
...
```{r creating new df with imputed values and writing csv, warning = F, message = F, fig.height = 2, fig.width = 6, include = FALSE}
# Empty data frame
...
# write_csv
# readr::write_csv(departures,"data/hflights_impute.csv")
```
...
```{r load original dataset, include=FALSE}
library(tidyverse)
...
```{r save new df, include=FALSE}
# readr::write_csv(dat, "data/polinfo.csv")
```
...
```{r saving model results to data folder, include=FALSE}
# readr::write_csv(df_mod1, "data/polinfo_mod1results.csv")
```
...
(**Not shown in class**) Below, I estimate a new model, `mod2` that includes the squared `age` variable. Note that I add an indicator for the model number `modnum` that we will later use to visually distinguish the results from both models.
```{r, warning = F, message = F, fig.height = 3, fig.width = 3}
dat <- dat %>%
  mutate(age2 = age^2)
